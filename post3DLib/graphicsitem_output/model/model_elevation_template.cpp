...
ModelElevationTemplate::ModelElevationTemplate(const Analysis2DViewSettings& _setting, const FIGURE_TYPE_PROPERTY& type, QUuid uuid)
    : FigureModelTemplate(_setting, type, uuid)
{
}

void ModelElevationTemplate::createJointItem(OutputGrScene *scene, JointData *jd, int ino, const QPointF &pp)
{
    Q_UNUSED(ino);
    ...
    item = new OutputJointItemModel(this,DATATYPE::TPJOINT,STRUCTTYPE::NOSTRUCTTYPE, jd->getUuid(),  v3, v1);
    ...
    FrameType::FrameType::FRAMETYPE f_type = uad->getFrameTypeOfFloor(myFrameID);
    ...
    if ( isClosedFrame && qAbs(pp.x()) < 1.0e-3 ) { // 閉じた通り
        ...
        VEC3D v2(0.0, 0.0, 1.0);
        VEC3D v3 = CalcVector3D::crossNormalized(v1, v2);
        OutputGraphicsItem *item = new OutputJointItemModel(this,DATATYPE::TPJOINT,STRUCTTYPE::NOSTRUCTTYPE, jd->getUuid(),v3,v1,false);
...
void ModelElevationTemplate::createLineItem(OutputGrScene* scene, MemberData *md, QList<QPointF> points)
{
    OutputGraphicsItem *item = nullptr;
    UnifiedAnalysisData* uad = UnifiedFixedData::getInstance()->getAnalysisData();
    ...
    if (getOutFigType() == OUTFIGTYPE::MODEL  ) {
        ...
        item = new OutputLineItemModel( this,md->dataType(),md->structureType(*uad), md->getUuid(),
                                        points, md->getTotalLength(),  v3, v1);
        item->setDrawingStatusDirect();
...
void ModelElevationTemplate::createPlaneItem(OutputGrScene *scene, MemberData* md,QList<QPointF> points,bool isNormalDirection )
{
...
void ModelElevationTemplate::createSpringItem(OutputGrScene *scene, SpringData* sd,QList<QPointF> points )
{
    OutputGraphicsItem *item = nullptr;
...
QColor ModelElevationTemplate::nameColor(DATATYPE dataType,STRUCTTYPE material,ATTENTION_STATUS status) const
{
    Q_UNUSED(status);
    if ( dataType == DATATYPE::TPJOINT ) {
        return getViewSettings()->getJointNumberColor();
    } else {
        return figureColor(dataType,material,status);
    }
}

QColor ModelElevationTemplate::zwallColor() const
{
    return getViewSettings()->getZwallColor();
}
QColor ModelElevationTemplate::rigidColor() const
{
    return getViewSettings()->getRigidRangeColor();
}
QColor ModelElevationTemplate::panelColor() const
{
    return getViewSettings()->getPanelZoneColor();
}
QColor ModelElevationTemplate::boundaryTextColor() const
{
    return getViewSettings()->getBoundaryConditionColor();
}
int ModelElevationTemplate::boundaryTextSize() const
{
    return getViewSettings()->getBoundaryConditionSize();
}
bool ModelElevationTemplate::boundaryTextVisible() const
{
    return getViewSettings()->getBoundaryConditionVisible();
}

void ModelElevationTemplate::createJointItems(OutputGrScene *scene)
{
    UnifiedAnalysisData* uad = UnifiedFixedData::getInstance()->getAnalysisData();
...
void ModelElevationTemplate::createMemberItems(OutputGrScene *scene)
{
    UnifiedAnalysisData* uad = UnifiedFixedData::getInstance()->getAnalysisData();
