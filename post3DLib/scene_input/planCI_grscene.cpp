...
namespace post3dapp{
PlanCopyInterfaceGrScene::PlanCopyInterfaceGrScene(LIMITSELECTTERM term, UnifiedInputData* input, const Input2DViewSettings& setting, bool _syncSetting, QObject *parent )
    : InputGrScene(term, SCENETYPE::PLANSCENE,setting, _syncSetting, parent), copy(input)
{
    backGround = new CopyBackGroundFloor(setting.UseScaledFont(), copy);
...
PlanCopyInterfaceGrScene::~PlanCopyInterfaceGrScene()
{
    delete backGround;
}
void PlanCopyInterfaceGrScene::setStatus(const QUuid &uid )
{
...
void PlanCopyInterfaceGrScene::setBackGround(const QString &fl, const QString &)
{
    qDebug() << "init setbackground";
    myFloorID = copy->nameToID(fl, DATATYPE::TPFLOOR);
    backGround->resetFramePoints(myFloorID);
    backGround->setColor(viewSettings.getScaleColor(),
...
void PlanCopyInterfaceGrScene::drawBackGround(QPainter *painter)
{
    qDebug() << "init drawbackground";
    backGround->drawWhole(painter);
}

QList<GlobalPoint> PlanCopyInterfaceGrScene::getCrossPoints() const
{
    return backGround->getCrossPoints();
}

QList<GlobalPoint> PlanCopyInterfaceGrScene::getDefinePoints() const
{
    return backGround->getDefinePoints();
}

void PlanCopyInterfaceGrScene::setCurrentFrames(const QList<QUuid> &idlist)
{
    static_cast<CopyBackGroundFloor*>(backGround)->setCurrentFrame(idlist);
    update();
}

void PlanCopyInterfaceGrScene::clearCurrentAttach()
{
    static_cast<CopyBackGroundFloor*>(backGround)->clearCurrentFrame();
    update();
}

void PlanCopyInterfaceGrScene::completeGlobalXYZ(GlobalPoint &gp)
{
    backGround->completeGlobalXYZ(gp);
}

void PlanCopyInterfaceGrScene::prepareBatchInputLines(DATATYPE tp)
{
    batchInputLines.clear();
    if ( tp == DATATYPE::TPCOLUMN || tp == DATATYPE::TPISO ) {
        batchInputLines = static_cast<CopyBackGroundFloor*>(backGround)->calcBatchColumnLines();
    } else if ( tp == DATATYPE::TPGIRDER ) {
        QList<FRAMEPOINTSVALUE> fplist = copy->getFrameValuesOfFloor(myFloorID);
            ...
            Q_FOREACH ( GlobalPoint gp, backGround->getCrossPoints() ) {
                if ( gp.containsFrame(fv.UUID) ) {
                    ...
                    if ( !isOpen && qAbs(xx) < 1.0e-3 ) {
                        int idx = -1;
                        qreal xz = copy->globalZToClosedFrameEndX(fv.UUID, floorHeight());
                        for (int i = 0; i < lenlist.count(); i++) {
...
void PlanCopyInterfaceGrScene::prepareBatchInputPlanes(bool containsBeam)
{
...
QList<MemberData *> PlanCopyInterfaceGrScene::filterSceneMember(const QList<MemberData *> &mdlist) const
{
...
void PlanCopyInterfaceGrScene::resetItems()
{
    ...
    QList<JointData *> jdlist =  copy->jointListOfFloor(myFloorID);
    createJointItems(jdlist);
    QList<MemberData *> mdlist =  copy->memberListOfFloor(myFloorID);
    createMemberItems(mdlist);

    update();
}

QUuid PlanCopyInterfaceGrScene::floorID() const
{
    return myFloorID;
}

qreal PlanCopyInterfaceGrScene::floorHeight() const
{
    return copy->getFloorHeight(myFloorID);
}

void PlanCopyInterfaceGrScene::createJointItems(const QList<JointData *> &jdlist)
{
...
void PlanCopyInterfaceGrScene::createMemberItems(const QList<MemberData *> &mdlist)
{
...
void PlanCopyInterfaceGrScene::changeMemberItems(const QList<MemberData *> &mdlist)
{
...
void PlanCopyInterfaceGrScene::createHLineItem(MemberData *md, unsigned int sflag, unsigned int vflag,
...
void PlanCopyInterfaceGrScene::createVLineUpperItem(MemberData *md, unsigned int sflag,
                                                unsigned int vflag,
...
void PlanCopyInterfaceGrScene::createVLineDownItem(MemberData *md, unsigned int vflag,
...
void PlanCopyInterfaceGrScene::createHPlaneItem(MemberData *md, unsigned int sflag, unsigned int vflag,
...
void PlanCopyInterfaceGrScene::createVPlaneItem(MemberData *md, unsigned int sflag, unsigned int vflag,
                                            const QList<QList<QPointF> > &planePoints, bool isOnPlane)
{
...
FIGURE_TYPE_PROPERTY PlanCopyInterfaceGrScene::getFigureTypeProperty() const{
    FIGURE_TYPE_PROPERTY type;
    type.type = OUTFIGTYPE::PLAN;
