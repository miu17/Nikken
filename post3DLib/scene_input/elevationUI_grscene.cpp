...
namespace post3dapp{
ElevationInterfaceGrScene::ElevationInterfaceGrScene(LIMITSELECTTERM term, const Input2DViewSettings& setting, bool _syncSetting, QObject *parent )
    : InputGrScene(term, SCENETYPE::ELEVATIONSCENE,setting, _syncSetting, parent)
{
    backGround = new BackGroundFrame(setting.UseScaledFont(), UnifiedInputData::getInInstance());
    ...
    connect( UnifiedInputData::getInInstance(), &UnifiedData::jointAppended,
            this, &ElevationInterfaceGrScene::createJointItems);
...
ElevationInterfaceGrScene::~ElevationInterfaceGrScene()
{
    delete backGround;
}

void ElevationInterfaceGrScene::setStatus(const QUuid &uid )
{
...
void ElevationInterfaceGrScene::setBackGround(const QString &fr, const QString &)
{
    qDebug() << "ElevationInterfaceGrScene::setBackGround : in";

    myFrameID =  UnifiedInputData::getInInstance()->nameToID(fr, DATATYPE::TPFRAMEP);
    backGround->resetFramePoints(myFrameID);
    backGround->setColor(viewSettings.getScaleColor(),
                        viewSettings.getScaleFocusColor());
    this->resetItems();
    update();
    qDebug() << "ElevationInterfaceGrScene::setBackGround : out";
}

void ElevationInterfaceGrScene::drawBackGround(QPainter *painter)
{
...
QList<GlobalPoint> ElevationInterfaceGrScene::getCrossPoints() const
{
    return backGround->getCrossPoints();
}

QList<GlobalPoint> ElevationInterfaceGrScene::getDefinePoints() const
{
    return backGround->getDefinePoints();
}

void ElevationInterfaceGrScene::setCurrentFloors(const QList<QUuid> &idlist)
{
    static_cast<BackGroundFrame*>(backGround)->setCurrentFloors(idlist);
    update();
}

void ElevationInterfaceGrScene::setCurrentFrames(const QList<QUuid> &idlist)
{
    backGround->setCurrentFrames(idlist);
    update();
}

void ElevationInterfaceGrScene::clearCurrentAttach()
{
...
void ElevationInterfaceGrScene::completeGlobalXYZ(GlobalPoint &gp)
{
    backGround->completeGlobalXYZ(gp);
}

void ElevationInterfaceGrScene::prepareBatchInputLines(DATATYPE tp)
{
    bool isOutput = false;
    ...
    if ( tp == DATATYPE::TPCOLUMN ) {
        ...
        QList<QUuid> fllist = UnifiedInputData::getInInstance()->getFloorListOfFrame(myFrameID);
        QList<QUuid> s_fllist = UnifiedInputData::getInInstance()->sortFloorOrderOfHeight(fllist);
...
void ElevationInterfaceGrScene::prepareBatchInputPlanes(bool containsBeam)
{
...
QList<MemberData *> ElevationInterfaceGrScene::filterSceneMember(const QList<MemberData *> &mdlist)
const
{
...
void ElevationInterfaceGrScene::resetItems()
{
    batchInputPlanes.clear();
    ...
    QList<JointData *> jdlist =  UnifiedInputData::getInInstance()->jointListOfFrame(myFrameID);
    createJointItems(jdlist);
    QList<MemberData *> mdlist =  UnifiedInputData::getInInstance()->memberListOfFrame(myFrameID);
    createMemberItems(mdlist);

    update();
}

QUuid ElevationInterfaceGrScene::frameID() const
{
    return myFrameID;
}

void ElevationInterfaceGrScene::createJointItems(const QList<JointData *> &jdlist)
{
    ...
    Q_FOREACH (JointData *jd, jdlist) {
        if ( jd->containsFrame(myFrameID) ) {
            ...
            XYZ pp = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID, jd->xyz());
            ...
            if ( isClosed && qAbs(pp.x) < 1.0e-3 ) {
                qreal cx =  UnifiedInputData::getInInstance()->globalZToClosedFrameEndX(myFrameID, jd->zPos());
...
/*
void ElevationInterfaceGrScene::changeJointItems(const QList<JointData*>& jdlist)
{

    bool isSelectable= ( viewSettings.currentElement()==ELJOINT &&
                         viewSettings.currentAddEditMode()==EDITMODE ) ;

    bool judgeLimitSelect=( viewSettings.currentElement()==ALLELEMENT );
    LIMITSELECTTERM selectTerm= FactoryUnifiedData::getInstance(isOutput)->currentLimitSelectTerm();
    bool isTypeLimitOk=true;
    if( judgeLimitSelect ){
        if( selectTerm.isSelectLimited ){
            if( selectTerm.isLimitDataType && !selectTerm.limitedDataTypes.contains(TPJOINT) ){
                isTypeLimitOk=false;
            }
            if( isTypeLimitOk && selectTerm.isLimitName ) isTypeLimitOk=false;
        }
    }
    ...
    Q_FOREACH(JointData* jd, jdlist){
        if( jointAndPoint.values().contains(jd) ){
            if( !jd->containsFrame(myFrameID) ){
                ...
                QList<JointItem*> j_items =jointAndPoint.keys(jd);
                XYZ pp= FactoryUnifiedData::getInstance(isOutput)->globalXYZToFrameXYZ(myFrameID,jd->xyz());
                ...
                jointAndPoint.key(jd)->setPos(QPointF(pp.x*1000.,pp.y*1000.));
                jointAndPoint.key(jd)->setZValue(pp.z*1000.0);
                jointAndPoint.key(jd)->setInteriorOn(jd->isInteriorJoint());
            }
        }else{
            {
                int ino= FactoryUnifiedData::getInstance(isOutput)->indexOfJoint(jd)+1;
                JointItem* item = new JointItem(ino);
                this->addItem(item);
                XYZ pp= FactoryUnifiedData::getInstance(isOutput)->globalXYZToFrameXYZ(myFrameID,jd->xyz());
                item->setPos(QPointF(pp.x*1000.,pp.y*1000.));
                item->setZValue(pp.z*1000.0);
                ...
                if( judgeLimitSelect ){
                    if( isTypeLimitOk ){
                        item->setFlag(QGraphicsItem::ItemIsSelectable,isSelectableJoint(jd,selectTerm));
                    }else{
                        item->setFlag(QGraphicsItem::ItemIsSelectable,false);
                    }
                }else{
                ...
                item->setVisible(viewSettings.getElementView(ELJOINT).isVisible);
                item->setSelected(false);
                item->setInteriorOn(jd->isInteriorJoint());
                jointAndPoint.insert(item,jd);
            }
...
void ElevationInterfaceGrScene::createMemberItems(const QList<MemberData *> &mdlist)
{
    bool isOutput = false;
    ...
    Q_FOREACH (MemberData *md, mdlist) {
        ...
        if ( md->isLineMember() ) {
            ...
            if ( md->dataType() == DATATYPE::TPCOLUMN ) {
                ...
                fr_angle = UnifiedInputData::getInInstance()->globalXYZToFrameAngle(myFrameID, xyz);
                qreal shy = md->getShiftY();
                qreal shz = md->getShiftZ();
                if ( qAbs(shy) > 1.0e-3 || qAbs(shz) > 1.0e-3 ) {
                    qreal x1 = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID, xyz).x;
                    xyz.x = xyz.x + shy / 1000.0;
                    xyz.y = xyz.y + shz / 1000.0;
                    qreal x2 = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID, xyz).x;
            ...
            if ( jdlist.first()->containsFrame(myFrameID) ) {
                ...
                if ( !jd1->containsFrame(myFrameID) ) {
                    XYZ xyz = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID, jd1->xyz());
                    if ( xyz.z < 0.0 ) {
        ...
        if ( md->isClosedMember() ) {
            ...
            for ( int i = 0; i < planePoints.count(); i++ ) {
                ...
                if ( isOnLine ) {
                    ...
                    XYZ p_xyz = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID, pj->xyz());
                    XYZ n_xyz = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID, nj->xyz());
...
void ElevationInterfaceGrScene::changeMemberItems(const QList<MemberData *> &mdlist)
{
    Q_FOREACH ( MemberData *md, mdlist ) removeMemberItem( md, md->dataType() );
    createMemberItems(filterSceneMember(mdlist));
}

void ElevationInterfaceGrScene::createHLineItem(MemberData *md, unsigned int sflag,
                                                unsigned int vflag,
...
void ElevationInterfaceGrScene::createVLineUpperItem(MemberData *md, unsigned int sflag,
                                                     unsigned int vflag,
                                                     const QList<int> &up_index, qreal angle, qreal shift)
{
    ...
    for (int i = 0; i < up_index.count(); i++) {
        ...
        if ( up_index.at(i) == 0 ) {
            ...
            XYZ xyz = UnifiedInputData::getInInstance()->globalXYZToFrameXYZ(myFrameID,
...
void ElevationInterfaceGrScene::createVLineDownItem(MemberData *md, unsigned int vflag,
...
void ElevationInterfaceGrScene::createHPlaneItem(MemberData *md, unsigned int sflag,
                                                 unsigned int vflag,
...
void ElevationInterfaceGrScene::createVPlaneItem(MemberData *md, unsigned int sflag,
                                                 unsigned int vflag,
                                                 const QList<QList<QPointF> > &planePoints, bool isOnPlane)
{
...
bool ElevationInterfaceGrScene::isClosedFrame() const
{
    FrameType::FRAMETYPE f_type =  UnifiedInputData::getInInstance()->getFrameTypeOfFloor(myFrameID);
    return ( f_type == FrameType::FRAMETYPE::CLCIRCLE || f_type == FrameType::FRAMETYPE::CLVARIABLE ) ;
}

FIGURE_TYPE_PROPERTY ElevationInterfaceGrScene::getFigureTypeProperty() const{
    FIGURE_TYPE_PROPERTY type;
    type.type = OUTFIGTYPE::SECTIONINPUT;
    QUuid uuid = this->frameID();
    type.viewtype=MODELVIEWTYPE::INPUT2DMODEL;
    type.scenetype=SCENETYPE::ELEVATIONSCENE;
    type.name=UnifiedInputData::getInInstance()->idToName(uuid, DATATYPE::TPFRAMEP);
