...
const QString WritePlan::Command = u8"*PLAN";
const QString WritePlan::Japanese = u8"伏図";

WritePlan::WritePlan(const QStringList& list):AbstractWrite(list){};

QString WritePlan::context(CalculationMakerLib::CalculationMaker *maker, const OutputPaperSetting& setting) const{
    ...
    for (int i = 1; i < list.count(); i++){
        for (int j = 0; j < 8; j++){
            ...
            maker->setFigureClass(new CalculationMakerLib::DrawInputFigure(figureName, true, option, Input2DViewSettings::makeSymbolSetting(setting.input2DViewSetting)));
