...
namespace post3dapp{
OperationWallPage::OperationWallPage( QWidget *parent )
    : AbstractElementPage( parent )
{
    myElementType = ELWALL;

    ReferenceCombobox *fugo_combo = new ReferenceCombobox(DATATYPE::TPWALL, UnifiedDataType::Input, this);
    ...
    connect(myButtonGroup, QOverload<int>::of(&QButtonGroup::buttonClicked),
            this, &OperationWallPage::slotChangeStackLayout);
    ...
    vlay1->addSpacing(5);
    vlay1->addWidget(wallStackedWidget);
...
void OperationWallPage::createStackedLayout()
{
    /* 周囲点指定 */
    QVBoxLayout *vlay1 = new QVBoxLayout;
    vlay1->setContentsMargins(0, 0, 0, 0);
    ...
    wallStackedWidget = new QStackedWidget(this);
    wallStackedWidget->addWidget(sub1_widget);
    wallStackedWidget->addWidget(sub2_widget);
    wallStackedWidget->addWidget(sub3_widget);
}

void OperationWallPage::slotChangeStackLayout(int id)
{
    signalCurrentMode(id);

    switch (id) {
    case POINTHITMODE:
        wallStackedWidget->setCurrentIndex(0);
        break;
    case ONEHITMODE:
        wallStackedWidget->setCurrentIndex(1);
        break;
    case BATCHMODE:
        wallStackedWidget->setCurrentIndex(2);
        break;
    default:
        wallStackedWidget->setCurrentIndex(0);
        break;
