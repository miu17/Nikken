...
#include "utility.h"
namespace post3dapp{
PersOutputSettingDialog::PersOutputSettingDialog(QWidget *parent)
    : QDialog(parent)
{
    createLayout();
...
void PersOutputSettingDialog::closeEvent(QCloseEvent *event)
{
    emit windowClosed();
    QDialog::closeEvent(event);
}

void PersOutputSettingDialog::setCenterPosition( qreal xx, qreal yy, qreal zz )
{
...
void PersOutputSettingDialog::setCurrentValues()
{
...
void PersOutputSettingDialog::createLayout()
{
    /* 背景 */
    ...
    connect( axisCheck, &QAbstractButton::toggled, this, &PersOutputSettingDialog::slotAxisValuesEnable );

    /* OK Cancel */
    QPushButton *okButton = new QPushButton(u8"変　更", this);
    QPushButton *cancelButton = new QPushButton(u8"キャンセル", this);

    connect( okButton, &QAbstractButton::clicked, this, &PersOutputSettingDialog::slotChangeViewSetting );
    connect( cancelButton, &QAbstractButton::clicked, this, &PersOutputSettingDialog::reject );
...
void PersOutputSettingDialog::slotAxisValuesEnable(bool onoff)
{
    axisLine->setEnabled(onoff);
}

void PersOutputSettingDialog::slotChangeViewSetting()
{
    bool ok = true;
