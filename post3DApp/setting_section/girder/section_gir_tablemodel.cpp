...
namespace post3dapp{
SectionGirderTableModel::SectionGirderTableModel(const SectionDataType &type, QObject *parent):
    SectionTableModel(type,parent)
{
}

SectionGirderTableModel::SectionGirderTableModel(const SectionGirderTableModel &model)
    :SectionTableModel(model){}


SectionValues SectionGirderTableModel::getMemberSection(int row) const
{
    return CreateLineMemberSection( getRowData(row), row );
}

QList<SectionValues> SectionGirderTableModel::getSectionList() const
{
...
LineMemberSection SectionGirderTableModel::CreateLineMemberSection(
        ...
        int currentrow) const
{
    ...
    if (isRC) {
        ...
        lm.line_param.i_haunch = myValues.at(coltag["iHAUNCH"]).toDouble();
        lm.line_param.j_haunch = myValues.at(coltag["jHAUNCH"]).toString().isEmpty()
                              ? lm.line_param.i_haunch : myValues.at(coltag["jHAUNCH"]).toDouble();
        //端中
        ...
        lm.line_param.j_haunch = myValues.at(coltag["jHAUNCH"]).toString().isEmpty()
                              ? lm.line_param.i_haunch : myValues.at(coltag["jHAUNCH"]).toDouble();
        lm.line_param.i_hasStiffener = ( myValues.at(coltag["iENDSTIFFENER"]).toString() == u8"有り");
        lm.line_param.j_hasStiffener = myValues.at(coltag["jENDSTIFFENER"]).toString().isEmpty()
                                    ? lm.line_param.i_hasStiffener
                                    : ( myValues.at(coltag["jENDSTIFFENER"]).toString() == u8"有り" );
